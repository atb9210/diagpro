<?php

namespace App\Filament\Admin\Resources;

use App\Filament\Admin\Resources\ImpostazioneResource\Pages;
use App\Filament\Admin\Resources\ImpostazioneResource\RelationManagers;
use App\Models\Impostazione;
use App\Services\OpenAIService;
use Filament\Forms;
use Filament\Forms\Form;
use Filament\Resources\Resource;
use Filament\Tables;
use Filament\Tables\Table;
use Filament\Notifications\Notification;
use Illuminate\Database\Eloquent\Builder;
use Illuminate\Database\Eloquent\SoftDeletingScope;

class ImpostazioneResource extends Resource
{
    protected static ?string $model = Impostazione::class;

    protected static ?string $navigationIcon = 'heroicon-o-cog-6-tooth';
    
    protected static ?string $navigationLabel = 'Impostazioni';
    
    protected static ?string $modelLabel = 'Impostazione';
    
    protected static ?string $pluralModelLabel = 'Impostazioni';
    
    protected static ?int $navigationSort = 99;

    public static function form(Form $form): Form
    {
        return $form
            ->schema([
                Forms\Components\Section::make('Dettagli Impostazione')
                    ->schema([
                        Forms\Components\TextInput::make('chiave')
                            ->required()
                            ->unique(ignoreRecord: true)
                            ->maxLength(255)
                            ->helperText('Identificativo univoco dell\'impostazione'),
                        Forms\Components\Select::make('tipo')
                            ->options([
                                'string' => 'Testo',
                                'number' => 'Numero',
                                'boolean' => 'Vero/Falso',
                                'json' => 'JSON',
                                'password' => 'Password/API Key',
                            ])
                            ->required()
                            ->live()
                            ->afterStateUpdated(fn (Forms\Set $set) => $set('valore', '')),
                        Forms\Components\TextInput::make('descrizione')
                            ->maxLength(255)
                            ->helperText('Descrizione dell\'impostazione'),
                    ])->columns(2),
                    
                Forms\Components\Section::make('Valore')
                    ->schema([
                        Forms\Components\TextInput::make('valore')
                            ->label('Valore')
                            ->required()
                            ->visible(fn (Forms\Get $get) => in_array($get('tipo'), ['string']))
                            ->maxLength(255),
                        Forms\Components\TextInput::make('valore')
                            ->label('API Key / Password')
                            ->required()
                            ->password()
                            ->revealable()
                            ->visible(fn (Forms\Get $get) => $get('tipo') === 'password')
                            ->maxLength(255)
                            ->helperText('Il valore sarà nascosto per sicurezza'),
                        Forms\Components\TextInput::make('valore')
                            ->label('Valore Numerico')
                            ->required()
                            ->numeric()
                            ->visible(fn (Forms\Get $get) => $get('tipo') === 'number'),
                        Forms\Components\Toggle::make('valore')
                            ->label('Valore Booleano')
                            ->required()
                            ->visible(fn (Forms\Get $get) => $get('tipo') === 'boolean'),
                        Forms\Components\Textarea::make('valore')
                            ->label('Valore JSON')
                            ->required()
                            ->visible(fn (Forms\Get $get) => $get('tipo') === 'json')
                            ->helperText('Inserire un JSON valido'),
                    ]),
            ]);
    }

    public static function table(Table $table): Table
    {
        return $table
            ->columns([
                Tables\Columns\TextColumn::make('chiave')
                    ->searchable()
                    ->sortable(),
                Tables\Columns\BadgeColumn::make('tipo')
                    ->colors([
                        'primary' => 'string',
                        'success' => 'number',
                        'warning' => 'boolean',
                        'info' => 'json',
                        'danger' => 'password',
                    ])
                    ->formatStateUsing(fn (string $state): string => match ($state) {
                        'string' => 'Testo',
                        'number' => 'Numero',
                        'boolean' => 'Vero/Falso',
                        'json' => 'JSON',
                        'password' => 'Password/API Key',
                        default => $state,
                    }),
                Tables\Columns\TextColumn::make('valore')
                    ->formatStateUsing(function ($state, $record) {
                        if ($record->tipo === 'password') {
                            return str_repeat('*', min(strlen($state), 20));
                        }
                        if ($record->tipo === 'boolean') {
                            return $state ? 'Sì' : 'No';
                        }
                        if ($record->tipo === 'json') {
                            return 'JSON (' . strlen($state) . ' caratteri)';
                        }
                        return $state;
                    })
                    ->limit(50)
                    ->toggleable(),
                Tables\Columns\TextColumn::make('descrizione')
                    ->limit(30)
                    ->toggleable(),
                Tables\Columns\TextColumn::make('updated_at')
                    ->dateTime()
                    ->sortable()
                    ->toggleable(isToggledHiddenByDefault: true),
            ])
            ->filters([
                Tables\Filters\SelectFilter::make('tipo')
                    ->options([
                        'string' => 'Testo',
                        'number' => 'Numero',
                        'boolean' => 'Vero/Falso',
                        'json' => 'JSON',
                        'password' => 'Password/API Key',
                    ]),
            ])
            ->actions([
                Tables\Actions\EditAction::make(),
                Tables\Actions\DeleteAction::make(),
                Tables\Actions\Action::make('test_openai')
                    ->label('Test Connessione')
                    ->icon('heroicon-o-wifi')
                    ->color('info')
                    ->visible(function ($record) {
                        // Debug: mostra sempre il pulsante per vedere se il problema è nella condizione
                        \Log::info('OpenAI button debug', [
                            'record_id' => $record->id,
                            'chiave' => $record->chiave,
                            'tipo' => $record->tipo,
                            'chiave_match' => $record->chiave === 'openai_api_key',
                            'tipo_match' => in_array($record->tipo, ['string', 'password'])
                        ]);
                        return $record->chiave === 'openai_api_key';
                    })
                    ->action(function ($record) {
                        try {
                            // Log per debug
                            \Log::info('Testing OpenAI connection from Filament', [
                                'record_id' => $record->id,
                                'chiave' => $record->chiave,
                                'tipo' => $record->tipo,
                                'has_value' => !empty($record->valore)
                            ]);
                            
                            if (empty($record->valore)) {
                                Notification::make()
                                    ->title('API Key non configurata')
                                    ->body('Inserisci prima una API Key valida per OpenAI.')
                                    ->danger()
                                    ->duration(5000)
                                    ->send();
                                return;
                            }
                            
                            $openAIService = app(OpenAIService::class);
                            $result = $openAIService->testConnection();
                            
                            \Log::info('OpenAI test result', $result);
                            
                            if ($result['success']) {
                                Notification::make()
                                    ->title('✅ Connessione riuscita!')
                                    ->body($result['message'])
                                    ->success()
                                    ->duration(5000)
                                    ->send();
                            } else {
                                Notification::make()
                                    ->title('❌ Errore di connessione')
                                    ->body($result['message'] . (isset($result['error']) ? ' (Codice: ' . $result['error'] . ')' : ''))
                                    ->danger()
                                    ->duration(8000)
                                    ->send();
                            }
                        } catch (\Exception $e) {
                            \Log::error('OpenAI test exception', [
                                'message' => $e->getMessage(),
                                'file' => $e->getFile(),
                                'line' => $e->getLine()
                            ]);
                            
                            Notification::make()
                                ->title('❌ Errore')
                                ->body('Errore durante il test: ' . $e->getMessage())
                                ->danger()
                                ->duration(8000)
                                ->send();
                        }
                    }),
            ])
            ->bulkActions([
                Tables\Actions\BulkActionGroup::make([
                    Tables\Actions\DeleteBulkAction::make(),
                ]),
            ]);
    }

    public static function getRelations(): array
    {
        return [
            //
        ];
    }

    public static function getPages(): array
    {
        return [
            'index' => Pages\ListImpostaziones::route('/'),
            'create' => Pages\CreateImpostazione::route('/create'),
            'edit' => Pages\EditImpostazione::route('/{record}/edit'),
        ];
    }
}